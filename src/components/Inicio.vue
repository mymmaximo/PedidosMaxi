  ...
  <div class="cuerpo">
    ...
    <Teleport to="body">
      <transition name="fade">
        <div v-if="ActualizarCajaPDel" @click.self="CerrarPopUp02" class="fondo">
          <div class="popup">
            <h1>¿Desear Eliminar/Reactivar {{ ProductoEli.nombre }}?</h1>
            <div>
              <div
                v-if="ProductoEli.imagenes.length > 0"
                class="flex flex-row gap-1 md:gap-3 overflow-x-auto items-center justify-center w-full md:pb-2 snap-x pb-1"
              >
                <button
                ...
                <div>
                  <img
                    ...
                    :src="
                      ObtenerImgUrl(ProductoEli.imagenes[GetImg(ProductoEli.id)].s3_key)
                    "
                    @load="ImagenesCargando[ProductoEli.id] = false"
                    class="imagen"
                  />
                  <div v-if="ImagenesCargando[ProductoEli.id] !== false" class="mt-2">
                    <img
            ...
            <div class="botones">
              <button @click="BorrarProducto()" class="botoncon">Si Confirmo</button>
              <button @click="CerrarPopUp02" class="botonc">Cancelar</button>
            </div>
          </div>
        </div>
      </transition>
    ...
    <Teleport to="body">
      <transition name="fade">
        <div v-if="VentanaCompra" @click.self="CerrarPopUp04" class="fondo">
          <div class="popup popup-compra">
            <h1>
              {{ ProductoActual.nombre }}
            </h1>
            <div class="carrusel-contenedor">
              <div v-if="ProductoActual.imagenes.length > 0" class="carrusel-fila">
                <button
                ...
                <div class="carrusel-img-caja">
                  <img
                    ...
                    :src="
                      ObtenerImgUrl(
                        ProductoActual.imagenes[GetImg(ProductoActual.id)].s3_key
                      )
                    "
                ...
                <button
                  ...
                  :disabled="
                    GetImg(ProductoActual.id) === ProductoActual.imagenes.length - 1
                  "
                  class="carrusel-btn"
                >
                  ❯
            ...
            <div class="panel-compra">
              <div class="control-cantidad">
                ...
                <input
                  ...
                  class="control-input"
                />
                <button @click="SumarProducto(ProductoActual)" class="control-btn-sumar">
                  ✚
                </button>
              </div>
              <div class="total-caja">
                <span class="total-subtitulo"> Precio Total </span>
            ...
            <div class="botones mt-2">
              <button @click="SumarCarrito" class="botoncon">Agregar al Carrito</button>
              <button @click="CerrarPopUp04" class="botonc">Cancelar</button>
            </div>
          </div>
        </div>
      </transition>
    ...
    <Teleport to="body">
      <transition name="slide">
        <div v-if="MostrarConfir" class="notificacion">
    ...
    <Teleport to="body">
      <transition name="fade">
        <div v-if="ActualizarCajaP" @click.self="CerrarPopUp01" class="fondo">
          <div class="popup">
            <form @submit.prevent="ActualizarProducto">
              ...
              <div v-if="VerificarRol([1, 2])">
                ...
                <select v-model="OpcionCategoriaA" class="seleccion">
                  <option value="new">+ Agrega una Categoria</option>
                  <option
              ...
              <div v-if="VerificarRol([1, 2])">
                ...
                <div
                  ...
                  class="flex flex-row md:gap-3 gap-1 items-center justify-center w-full md:pb-2 pb-1"
                >
                  <button
                  ...
                  <div class="relative w-fit mx-auto mt-2">
                    ...
                    <div>
                      <img
                        ...
                        :src="
                          ObtenerImgUrl(
                            ProductoAct.imagenes[GetImg(ProductoAct.id)].s3_key
                          )
                        "
                        @load="ImagenesCargando[ProductoAct.id] = false"
                        :class="
                          DelImg.includes(
                            ProductoAct.imagenes[GetImg(ProductoAct.id)].id_imagen
                          )
                            ? 'imagendel'
                            : 'imagen'
                        "
                      />
                      <div v-if="ImagenesCargando[ProductoAct.id] !== false" class="mt-2">
                        <img
                          src="../assets/loading.gif"
                          alt="Cargando..."
                          class="imagen !2xl:p-15"
                ...
                <div v-else class="imageno">Sin imágenes</div>
                <!-- Imagenes Nuevas -->
                <div>
                  <h2>Imagenes Nuevas</h2>
                  <div v-if="VistaPrevia.length > 0" class="relative w-fit mx-auto mt-2">
                    <div
                    ...
                    >
                      ...
                      <img :src="img" alt="Vista Previa" class="imagen !m-0" />
                    </div>
                  </div>
              ...
              <div class="botones">
                <button
                  :disabled="confirboton || Actualizando"
                  type="submit"
                  class="botoncon"
                >
                  {{ Actualizando ? "Actualizando..." : "Actualizar" }}
                </button>
                <button @click="CerrarPopUp01" type="button" class="botonc">
                  Cancelar
                </button>
              </div>
            </form>
    ...
    <Teleport to="body">
      <transition name="fade">
        <div v-if="VentanaBanner" @click.self="CerrarPopUp03" class="fondo">
          <div class="popup">
            <h1 class="text-center">Banners:</h1>
            <!-- Subir Banners -->
            <div>
              <h1>Nuevos Banners</h1>
              <div class="md:mt-4 mt-2">
                <input
                  ...
                  <template v-if="banner.id">
                    ...
                    <div v-if="bannerdesactivado(banner)">
                      ...
                      <button
                        v-else
                        @click="
                          DelSupaBann = DelSupaBann.filter((id) => id !== banner.id)
                        "
                        type="button"
                    ...
                    <div class="banner-wrapper">
                      ...
                      />
                      <div v-if="bannerdesactivado(banner)" class="banner-overlay"></div>
                    </div>
                  </template>
                  <template v-else>
                    <h1>Nuevo!</h1>
            ...
            <div class="botones">
              <button @click="SaveBanner" class="botoncon">Guardar Cambios</button>
              <button @click="CerrarPopUp03" class="botonc">Cancelar</button>
            </div>
          </div>
        </div>
      </transition>
    ...
    <Teleport to="body">
      <transition name="fade">
        <div v-if="VentanaPromocion" @click.self="CerrarPopUp05" class="fondo">
          <div class="popup !max-w-md">
            <form @submit.prevent="GuardarPromocion" class="flex flex-col gap-4">
              <h1 class="text-center !text-xl !mb-2">
                🏷️ Oferta: {{ ProductoPromo.nombre }}
              </h1>
              <div
                class="bg-gray-50 rounded-xl p-2 shadow-inner mx-auto w-full max-w-[200px]"
              >
                <div
                  v-if="ProductoPromo.imagenes.length > 0"
                ...
                >
                  ...
                  <div
                    class="shrink-0 flex justify-center w-24 h-24 relative rounded-lg overflow-hidden border-2 border-white shadow-sm"
                  >
                    <img
                      v-show="ImagenesCargando[ProductoPromo.id] === false"
                      :src="
                        ObtenerImgUrl(
                          ProductoPromo.imagenes[GetImg(ProductoPromo.id)].s3_key
                        )
                      "
                    ...
                    />
                    <div
                      v-if="ImagenesCargando[ProductoPromo.id] !== false"
                  ...
                  <button
                    ...
                    :disabled="
                      GetImg(ProductoPromo.id) === ProductoPromo.imagenes.length - 1
                    "
                    class="botonflecha !p-1 !h-8 !w-8 shrink-0"
                  >
                    ❯
              ...
              <div
                class="flex flex-row justify-center gap-2 mt-2 pt-4 border-t-2 border-gray-100"
              >
                <button
                  type="submit"
                ...
                <button @click="CerrarPopUp05" type="button" class="botonc flex-1 !m-0">
                  Cancelar
                </button>
              </div>
            </form>
            ...
            >
              ...
              >
                <button
                  ...
                  type="button"
                  class="botonflechagrande absolute left-0 top-0 z-20 opacity-0 hidden md:flex group-hover:opacity-100"
                >
                  ❮
                </button>
                <button
                ...
                <div>
                  <img
                    ...
                    class="banner"
                  />
                  <div v-if="ImagenesCargando[Bananaer.id] !== false" class="mt-2">
                    <img src="../assets/loading.gif" alt="Cargando..." class="banner" />
                  </div>
                </div>
                <button
                  @click="IndiceBanner++"
                  ...
                  type="button"
                  class="botonflechagrande absolute right-0 top-0 z-20 opacity-0 hidden md:flex group-hover:opacity-100"
                >
                  ❯
                </button>
              </div>
            </div>
            <div v-for="cat in ListaCategoria" :key="cat.categoria">
              <div
                v-if="
                  ProductosPorCategoria[cat.categoria] &&
                  ProductosPorCategoria[cat.categoria].length > 0
                "
                class="relative group flex flex-col md:mb-8 mb-2 bg-gradient-to-tr from-green-600/50 to-green-300/50 rounded-2xl"
              >
                <h1
                  class="text-3xl font-extrabold text-green-900 md:mb-2 mb-0 drop-shadow-sm"
                >
                  {{ cat.categoria }}
                </h1>
                <div
                  :id="'carrusel-' + cat.categoria"
                  class="flex flex-row w-full overflow-x-auto md:overflow-hidden md:py-4 py-2 scroll-smooth gap-5"
                >
                  <button
                    @click="CarruselIzquierda(cat.categoria)"
                    :disabled="ObtenerIndiceCarrusel(cat.categoria) === 0"
                    class="botonflechacatr absolute left-0 top-[55%] hidden md:flex opacity-0 group-hover:opacity-100"
                  >
                    ❮
                  </button>
                  <div
                  ...
                  >
                    <div>
                      ...
                      <div
                        ...
                        class="flex flex-row 2xl:gap-3 gap-1 items-center justify-center w-full md:pb-2 pb-1 snap-x"
                      >
                        <img
                      ...
                      <div>
                        ...
                        <div class="flex flex-row lg:flex-col justify-center">
                          <div
                            class="flex flex-wrap w-full mt-auto justify-center items-center pt-3 gap-1.5 sm:gap-2"
                          >
                            <button
                              @click.stop="Edicion(i)"
                              v-if="VerificarRol([1, 2, 4, 5])"
                              class="botont !px-2 !py-2 !text-sm"
                            >
                              ✏️
                              <span class="hidden xl:inline ml-1 truncate"> Editar </span>
                            </button>
                            <button
                              @click.stop="AbrirPopUp05(i)"
                  ...
                  <button
                    @click="
                      CarruselDerecha(
                        cat.categoria,
                        ProductosPorCategoria[cat.categoria].length
                      )
                    "
                    class="botonflechacatl absolute right-0 top-[55%] hidden md:flex opacity-0 group-hover:opacity-100"
                  >
                    ❯
                  </button>
                </div>
...
const bannersorden = computed({
  get() {
    const viejo =
      Bananaeract.value && Bananaeract.value.imagenes ? Bananaeract.value.imagenes : [];
...
const bannerdesactivado = (banner) => {
  if (!banner.id) return false;
  if (banner.activo === true) return DelBann.value.includes(banner.id);
  return !DelBann.value.includes(banner.id);
};
const bannerestatus = (id) => {
  if (DelBann.value.includes(id)) {
    DelBann.value = DelBann.value.filter((b) => b !== id);
...
const NoMoreImages = (img) => {
  const index = DelImg.value.indexOf(img.id_imagen);
  if (index === -1) {
...
const ObtenerImgUrl = (Imgenkey) => {
  const respuesta = supabase.storage.from("max_imagenes").getPublicUrl(Imgenkey);
...
const ActualizarProducto = async () => {
  ...
  try {
    ...
    const ActProducto = await fetch(
      `${urlover8000}/productos/id/${ProductoAct.value.id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    for (const id_img of DelImg.value) {
      ...
      const ImgDel = listaImagenes.find((img) => img.id_imagen === id_img);
...
const BorrarBanner = async () => {
  ...
  for (const id_banner of DelSupaBann.value) {
    const BannerDelete = Bananaeract.value.imagenes.find((b) => b.id === id_banner);
    if (BannerDelete) {
...
const BorrarProducto = async () => {
  const EraseProducto = await fetch(
    `${urlover8000}/productos/id/${ProductoEli.value.id}`,
    {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
...
const BorrarPromocion = async () => {
  ...
  try {
    const respuesta = await fetch(
      `${urlover8000}/registro_precios/id/${FormPromo.value.id_promocion}`,
      {
        method: "DELETE",
        credentials: "include",
      }
...
const Confirmar = async () => {
  ...
  if (PedidoActual.value) {
    ...
    if (respuesta.ok) {
      ...
      localStorage.setItem("pedido", PedidoActual.value);
      Confirmar();
    } else {
      const error = await respuesta.text();
...
const GuardarPromocion = async () => {
  ...
  try {
    const payload = {
      ...
      precio_nuevo: parseFloat(FormPromo.value.precio_nuevo),
...
const SumarCarrito = () => {
  ...
  let CarritoExistente = CarritoLocal.value.find(
    (item_exitente) => item_exitente.id_producto === ProductoActual.value.id
  );
  if (CarritoExistente) {
...
const ToggleFavorito = async (id_producto) => {
  try {
    const respuesta = await fetch(
      `${urlover8000}/favoritos/toggle?id_cliente=${ClienteID.value}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
